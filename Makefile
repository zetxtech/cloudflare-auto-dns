...
help/simple:
>   @echo "Welcome to cloudflare-auto-dns!"
>   @echo "\nUsage: make <subcommand>"
>   @echo "  install - create a venv and install cloudflare-auto-dns in it."
>   @echo "  develop - install cloudflare-auto-dns and also dev-related tools."
>   @echo "  run - run cloudflare-auto-dns with config.yml."
...
>   @echo "\nFor using cloudflare-auto-dns, you can run:"
...
>   @echo "  run - run githubstar api server with config.toml."
>   @echo "  run/server - run githubstar api server with config.toml."
>   @echo "  lint - fix code style with black and pre-commit."
>   @echo "  test - run pytest using current python."
>   @echo "  debugpy - start cloudflare-auto-dns with debugpy (vscode debug module) for remote connection at localhost:5678."
...
>   "$(VENV)/bin/python" -m pip install -i "$(PYPI_URL)" -r requirements.txt \
>   && echo "Info: cloudflare-auto-dns has been installed successfully." \
>   && echo 'Info: Run "make run" to run cloudflare-auto-dns.' \
>   && echo 'Info: Run "make systemd" to enable autostart.' \
>   || echo "Error: Fail to install cloudflare-auto-dns."
